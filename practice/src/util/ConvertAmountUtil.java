...
public class ConvertAmountUtil {
	 ...
	 * 金额格式化
	 */
	public static String getFormatMoney(double a) {
	...
	public static double add(double v1, double v2) {
		...
		return C1.setScale(SCALE_LENGTH, BigDecimal.ROUND_HALF_UP).doubleValue();
	}
	 ...
	 * @return 两个参数的和
	 */
	public static double addNoScale(double v1, double v2) {
		BigDecimal b1 = new BigDecimal(Double.toString(v1));
		BigDecimal b2 = new BigDecimal(Double.toString(v2));
		BigDecimal C1 = b1.add(b2);
	...
	/**
	 * 提供手续费取得运算。 小数点后保留位数：2位 精度取值方式：ROUND_HALF_UP 四舍五入
	 * 
	 * @param v1
	 *            金额
	 * @param v2
	...
	public static double getRate(double v1, double v2) {
		...
		BigDecimal DivideResult = mulResult.divide(new BigDecimal(RATE_TYPE), SCALE_LENGTH, BigDecimal.ROUND_HALF_UP);
	...
	public static String moneyYu(String data) {
		String payAmount = new BigDecimal(data).multiply(new BigDecimal(100)).setScale(0, BigDecimal.ROUND_DOWN)
				.toString();
	...
	public static Double parseAmt(String amtStr) {
		...
		try {
			...
			amtStr = amtStr.substring(0, amtStr.length() - 2) + "." + amtStr.substring(amtStr.length() - 2);
	...
	public static String getFormatAmount(String money) {
		// 没有小数点
		if (money.indexOf(".") == -1) {
			money += ".00";
		}
		...
		// 小数位不够
		int decimalLength = money.substring(money.indexOf(".") + 1).length();
